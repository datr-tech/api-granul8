...
import {
  IAttributeControllerReadAttributeOutputError as IControllerError,
  IAttributeControllerReadAttributeOutputSuccess as IControllerSuccess,
...
/**
 * @name					attributeRouterReadAttribute
 *
 * @description		The 'readAttribute' router for 'attribute', whose expected
 *                inputs have been defined within the following schema:
 *                'attributeValidationSchemaReadAttribute'.
 *
...
export const attributeRouterReadAttribute = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'attributeValidationSchemaReadAttribute'.
      ...
      const stat = await attributeController.readAttribute(validatedParams);
       ...
       * 'attributeModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { attributeModel, responseStatusCode } = (stat as IControllerSuccess).payload;
      res.status(responseStatusCode).send({ attributeModel });
    } catch (error) {
      /*
