...
import {
  IAttributeControllerDeleteAttributeOutputError as IControllerError,
  IAttributeControllerDeleteAttributeOutputSuccess as IControllerSuccess,
...
/**
 * @name					attributeRouterDeleteAttribute
 *
 * @description		The 'deleteAttribute' router for 'attribute', whose expected
 *                inputs have been defined within the following schema:
 *                'attributeValidationSchemaDeleteAttribute'.
 *
 ...
 *                controller, 'attributeController', which, when successful, will return
 ...
 *                | Server error                | 500               |
 */
export const attributeRouterDeleteAttribute = Router(options).delete(
  '/',
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'attributeValidationSchemaDeleteAttribute'.
       ...
       * to 'attributeController'.
       */

      const validatedParams = matchedData<IAttributeModel>(req);
      const stat = await attributeController.deleteAttribute(validatedParams);
       ...
       * 'attributeId' from 'stat.payload' and return
      ...
      res
        ...
        .send({ attributeId: controllerResponsePayload['attributeId'] });
    } catch (error) {
      /*
